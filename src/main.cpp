...
// MAX7219 pin mapping
// ============================
// We default to the S3 Super Mini pins. You can override these in platformio.ini Using build_flags.
#ifndef CLK_PIN
  #define CLK_PIN 18
  #define DATA_PIN 5
  #define CS_PIN 23
...
void updateProxyAuthHeader() {
  if (strlen(proxyUser) > 0) {
    String auth = String(proxyUser) + ":" + String(proxyPass);
    proxyAuthHeader = "Basic " + base64::encode(auth);
  } else {
    proxyAuthHeader = "";
  }
}
...
void loadConfig() {
  ...
  DynamicJsonDocument doc(4096);  // Size based on ArduinoJson Assistant + buffer
  ...
  proxyEnabled = doc["proxyEnabled"] | false;
  strlcpy(proxyServer, doc["proxyServer"] | "", sizeof(proxyServer));
  proxyPort = doc["proxyPort"] | 3128;
...
void connectWiFi() {
  ...
  // Keep SoftAP active (AP_STA mode) so 192.168.4.1 settings page is always accessible!
...
#ifdef ESP32
  WiFi.setSleep(false);
  WiFi.setTxPower(WIFI_POWER_15dBm);  // 15dBm prevents current spikes that trigger brownout detector
#endif
  setupHostname();
...
time_t utc_mktime(struct tm *tm) {
  ...
  if (is_leap_year && month < 2) leap_days--;

  long days = (year - 1970) * 365L + days_before_month[month] + tm->tm_mday - 1 + leap_days;
  return (time_t)(days * 86400L + tm->tm_hour * 3600L + tm->tm_min * 60L + tm->tm_sec);
}

time_t parseHttpDate(const String &dateStr) {
  if (dateStr.length() < 16) return 0;
  struct tm tmDate;
  memset(&tmDate, 0, sizeof(struct tm));
  // RFC 1123 format e.g. "Wed, 09 Sep 2026 14:15:30 GMT" or without day "09 Sep 2026 14:15:30 GMT"
  char *res = strptime(dateStr.c_str(), "%a, %d %b %Y %H:%M:%S", &tmDate);
  if (!res) {
    res = strptime(dateStr.c_str(), "%d %b %Y %H:%M:%S", &tmDate);
  }
  if (res != nullptr) {
    return utc_mktime(&tmDate);
  }
  return 0;
}

void applySyncedTime(time_t epoch, const char *source) {
  if (epoch > 1700000000) {  // Valid modern epoch (late 2023+)
    ...
    setenv("TZ", posixTz, 1);
    tzset();
    Serial.printf("[TIME] Successfully synced time (%s): %lu\n", source, (unsigned long)epoch);
  }
}
...
bool syncTimeViaHTTP() {
  ...
  Serial.println(F("[TIME] Attempting HTTP time sync..."));
  ...
  for (size_t i = 0; i < numEndpoints; i++) {
    ...
    const char *url = endpoints[i];
    if (proxyEnabled && strlen(proxyServer) > 0) {
      Serial.printf("[TIME] Querying [%s] via proxy %s:%d\n", url, proxyServer, proxyPort);
      http.begin(client, proxyServer, proxyPort, url, false);
      http.addHeader("Host", "connectivitycheck.gstatic.com");
      if (proxyAuthHeader.length() > 0) {
        http.addHeader("Proxy-Authorization", proxyAuthHeader);
      }
    } else {
      Serial.printf("[TIME] Querying [%s] directly\n", url);
      http.begin(client, url);
    }

    int httpCode = http.GET();
    Serial.printf("[TIME] Endpoint response code: %d\n", httpCode);

    // 1. Check HTTP Date header (returned by standard HTTP servers and proxies)
    if (http.hasHeader("Date")) {
      String dateStr = http.header("Date");
      Serial.printf("[TIME] Parsing Date header: %s\n", dateStr.c_str());
      time_t epoch = parseHttpDate(dateStr);
      if (epoch > 1700000000) {
        applySyncedTime(epoch, "HTTP Date header");
        http.end();
        return true;
      }
    }

    // 2. If worldtimeapi returned JSON payload with unixtime
    if (httpCode == 200 && strstr(url, "worldtimeapi.org") != nullptr) {
      String payload = http.getString();
      DynamicJsonDocument doc(1024);
      DeserializationError err = deserializeJson(doc, payload);
      if (!err && doc.containsKey("unixtime")) {
        time_t epoch = doc["unixtime"].as<time_t>();
        if (epoch > 1700000000) {
          applySyncedTime(epoch, "worldtimeapi JSON");
          http.end();
          return true;
        }
      }
  ...
  Serial.println(F("[TIME] HTTP time sync could not determine time from any endpoint."));
  return false;
}

void setupTime() {
  if (!isAPMode) {
    Serial.println(F("[TIME] Starting time synchronization"));
  }

  // Sanitize ntpServer2: If it begins with http:// or https:// (e.g. Nightscout hack), don't pass as NTP server
  const char *server2 = ntpServer2;
  String s2 = String(ntpServer2);
  if (s2.startsWith("http://") || s2.startsWith("https://") || s2.indexOf('/') != -1) {
    Serial.println(F("[TIME] Secondary NTP server is a Nightscout URL. Using time.google.com for secondary NTP."));
    server2 = "time.google.com";
  }

  const char *posixTz = ianaToPosix(timeZone);
#if defined(ESP32)
  configTzTime(posixTz, ntpServer1, server2);
#else
  configTime(0, 0, ntpServer1, server2);
  setenv("TZ", posixTz, 1);
  tzset();
#endif
...
void setupWebServer() {
  ...
  server.on("/config.json", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    doc[F("proxyEnabled")] = proxyEnabled;
    doc[F("proxyServer")] = String(proxyServer);
  ...
  server.on("/save", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    for (int i = 0; i < request->params(); i++) {
      ...
      else if (n == "proxyEnabled") doc[n] = (v == "true" || v == "on" || v == "1");
      else if (n == "proxyPort") doc[n] = v.toInt();
  ...
  server.on("/status", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    config["proxyServer"] = String(proxyServer);
...
void fetchWeather() {
  ...
  const char *weatherHeaderKeys[] = { "Date" };

  if (proxyEnabled && strlen(proxyServer) > 0) {
    plainClient.stop();
    yield();
    Serial.printf("[WEATHER] Connecting via HTTP proxy %s:%d...\n", proxyServer, proxyPort);
    http.begin(plainClient, proxyServer, proxyPort, url, false);
    http.addHeader("Host", "api.openweathermap.org");
    if (proxyAuthHeader.length() > 0) {
      http.addHeader("Proxy-Authorization", proxyAuthHeader);
    }
    http.setTimeout(10000);
    http.collectHeaders(weatherHeaderKeys, 1);
    Serial.println(F("[WEATHER] Sending GET request via proxy..."));
  } else {
#if defined(ESP8266) || defined(CONFIG_IDF_TARGET_ESP32S2)
    // ===== ESP8266 → HTTP =====
    plainClient.stop();
    ...
    secureClient.stop();
    yield();
    secureClient.setInsecure();  // no cert validation
  ...
  if (httpCode == HTTP_CODE_OK) {  // Check if HTTP response code is 200 (OK)
    ...
    String httpDateStr = "";
    if (http.hasHeader("Date")) {
      httpDateStr = http.header("Date");
    }

    String payload = http.getString();
    http.end();  // Release client connection before parsing and LittleFS operations
    Serial.println(F("[WEATHER] Response received."));
    Serial.print(F("[WEATHER] Payload: "));  // Use F() with Serial.print
    Serial.println(payload);

    // Sync system time using OpenWeather's real-time HTTP Date response header if available
    if (httpDateStr.length() > 0) {
      time_t httpDateEpoch = parseHttpDate(httpDateStr);
      if (httpDateEpoch > 1700000000) {
        time_t currentNow = time(nullptr);
        if (!ntpSyncSuccessful || abs((long)(currentNow - httpDateEpoch)) > 30) {
          applySyncedTime(httpDateEpoch, "OpenWeather HTTP Date header");
        }
      }
    }
    ...
    // Note: Do NOT use doc["dt"] to set the system clock, as dt is the weather observation
    // calculation timestamp which can be 15-60+ minutes old.
    if (doc.containsKey(F("dt"))) {
      time_t obsTime = doc[F("dt")].as<time_t>();
      Serial.printf("[WEATHER] Weather data observation timestamp: %lu\n", (unsigned long)obsTime);
    }
    ...
    Serial.printf("[WEATHER] Description: %s (icon char 0x%02X)\n", detailedDesc.c_str(), (uint8_t)weatherIcon[0]);
    ...
    if (doc.containsKey(F("sys"))) {
      ...
      if (sys.containsKey(F("sunrise")) && sys.containsKey(F("sunset"))) {
        ...
        // Ensure current TZ rule is applied
        const char *posixTz = ianaToPosix(timeZone);
        setenv("TZ", posixTz, 1);
        tzset();

        // Convert UTC directly to local time using standard localtime_r
        struct tm tmSunrise, tmSunset;
        localtime_r(&sunriseUtc, &tmSunrise);
        localtime_r(&sunsetUtc, &tmSunset);
...
*/
void setup() {
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0);  // Disable software brownout detector reset
  Serial.begin(115200);
  delay(1000);
  ...
  loadUptime();
  // Initialize Hardware SPI explicitly to the custom pins before Parola
...
void loop() {
  ...
  // Allow AP clients to continue resolving captive portal queries while clock runs
  dnsServer.processNextRequest();

  // Auto-reconnect station if dropped
  static unsigned long lastStaReconnectAttempt = 0;
  if (!isAPMode && WiFi.status() != WL_CONNECTED && credentialsExist()) {
    if (millis() - lastStaReconnectAttempt > 15000) {
      lastStaReconnectAttempt = millis();
      Serial.println(F("[WIFI] STA connection lost. Attempting to reconnect to WiFi..."));
      WiFi.reconnect();
    }
  }
  ...
  switch (ntpState) {
    ...
    case NTP_SYNCING:
      {
        time_t now = time(nullptr);
        if (now > 1700000000) {  // Valid NTP sync successful (epoch > late 2023)
          Serial.println(F("[TIME] NTP sync successful."));
          ntpSyncSuccessful = true;
          ntpState = NTP_SUCCESS;
        } else if (millis() - ntpStartTime > 4000 && proxyEnabled && !ntpSyncSuccessful) {
          syncTimeViaHTTP();
